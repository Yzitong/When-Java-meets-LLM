...
a）先删缓存的原因：主动清空旧缓存，让后续读请求 “缓存命中失败”，被迫去读数据库（避免直接读取旧缓存）
...
redis分布式锁需要设置超时时间：否则获取锁成功后出现服务器宕机情况，此时锁就没有办法释放，其他业务无法使用
...
zset （排序+去重）是 Redis 常用数据类型之一，它适用于排行榜类型的业务场景，比如 QQ 音乐排行榜、用户贡献榜等。在音乐排行榜单中，我们可以将歌曲的点击次数作为 score 值，把歌曲的名字作为 key 值，通过对 score 排序就可以得出歌曲“热度榜单”。
